...
//			document.write('\n' + '<link rel="stylesheet" type="text/css" href="'+ this.getPath() +'css/skin.css"  />' + '\n' + ie6PNG);
	...
	validator={
        ...
        DivMobile:/(^(13[0-9]|15[012356789]|18[0236789]|14[57])[0-9]{8}$)|(^(\d{4}|\d{3})-(\d{7,8})$)/,
        Mobile:/^0?(13[0-9]|15[012356789]|18[0236789]|14[57])[0-9]{8}$/,
        Tele:/^(1([\d]{10}))|((\([0-9]+\))?[0-9]{7,8})$/,
	    ...
	    Currency : /^\d+(\.\d+)?$/,
	    IsNoZero : /^(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*))$/, 
	    Number : /^\d+$/,
	    NumberAndEnglish :/^[A-Za-z0-9]+$/,
	    Zip : /^[1-9]\d{5}$/,
	    QQ : /^[1-9]\d{4,8}$/,
//		IP  : /^[\d\.]{7,15}$/,
        ...
        Select:/^[-1]{1}$/,
        code:/^\w+$/,//由数字、26个英文字母或者下划线组成的字符串
        Nickname:/^[\u4E00-\u9FA5\w\d]+$/,//字母、数字、下划线、汉字
        ChineseNumEnglish:/^[a-zA-Z0-9\u4e00-\u9fa5]+$/,
        InvoiceCode:/^\d{8}$/,
        invoiceDaima:/^\d{10}$/,
        startDate: "this.validStartDate(value)",		/* 验证开始日期 */
        endDate: "this.validEndDate(value)",		/* 验证结束日期 */
		...
		// 验证开始日期
		validStartDate: function(value) {
			// 如果当期日期为空，不进行校验，返回true
			if(value == '') return true;
			
			var element = this['element'];
			this['valiData'] = $.trim(element.attr('validata-options'));
			this['dataTypes']=eval("({"+this['valiData']+"})");
			var dataTypes=this['dataTypes'];
			
			// 如果开始日期为空，不进行校验，返回true
			var startDateStr = $(dataTypes.compareDate).val();
			if (startDateStr == '') 
				return true;
			...
			// 将值转换为Date
			var startDate = validator.str2Date(startDateStr, formatString);
			if (startDate == null) return false;
			var endDate = validator.str2Date(value, formatString);
		...
		validEndDate: function(value) {
			...
			// 如果结束日期为空，不进行校验，返回true
			var endDateStr = $(dataTypes.compareDate).val();
			if (endDateStr == '') 
				return true;
			...
			// 将值转换为Date
			var startDate = validator.str2Date(value, formatString);
			if (startDate == null) return false;
			var endDate = validator.str2Date(endDateStr, formatString);
			if (endDate == null) return false;
...
//					data = data.replace(/(^\s*)|(\s*$)/g, "");
	...
	// 将字符串转换为Date数据
	validator.str2Date = function (value, format) {
		var m, year, month, day;
		switch(format){
		case "ymd" :
	...
	validator.check=function(obj){
		...
		var msgs = dataTypes.msg.split('|');	// 增加多规则验证时提示不同错误信息，用|分割，add by wsz of 2016-07-01
		...
		jQuery.each(validType,function(index,type){
			...
			if(validator.checkDataType(obj,type)==false){ 
				//校验失败
				validator.showTipMsg(obj,msgs[index],index,false);
				return isValid=false;
			} else { 
				// 校验成功
				validator.showTipMsg(obj,msgs[index],index,true);		
				return isValid=true;
			}
		});
		return isValid;
